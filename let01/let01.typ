...
Далее создается новый пользователь `arseny`, затем директория создается вручную от имени root и передается в ownership пользователю. Команда `usermod` добавляет пользователя в группу `sudo` для выполнения административных задач.
