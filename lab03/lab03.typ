...
В рамках домашнего задания было необходимо реализовать веб-сервер для учёта лабораторных работ и летучек.

== Архитектура решения

Сервер реализован на Java с использованием встроенного HTTP-сервера `com.sun.net.httpserver`. Рендеринг HTML-страниц выполняется на стороне сервера — при каждом запросе формируется полная HTML-страница, которая отправляется клиенту. Для стилизации используется фреймворк Pico.css, подключаемый через CDN.

Сервер поддерживает:
- Отображение списка лабораторных работ и летучек;
...
#figure(
  ...
  caption: "Веб-интерфейс системы учёта лабораторных работ и летучек",
)
